...
const Select=(props)=>{
  ...
  return <div className={`${style.select} ${props.sty}`} >
    <select onBlur={onBlur} onChange={handleChange} required={required} id={id}  name={name} className={'form-select form-control'} >
      {!opnull?<option value="">Seleccionar...</option>:<></>}
    {
      Array.isArray(options)?options.map((g,index)=>{
        return g.value==defaultValue?
        <option selected  key={index} value={g.value} >{g.label}</option>
